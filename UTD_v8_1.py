...
# =====================================================================
# 💎 [V8.0 멀티 타임프레임 파라미터] - 30일 백테스트 1등 세팅 100% 이식
# =====================================================================
strategy_config = {
    # 👑 [1시간봉(minute60) 제왕 그룹] - 큰 파동을 묵직하게 먹는 코인
    "KRW-TAO":  {"interval": "minute60", "ma_short": 3, "ma_long": 15, "use_pullback": True,  "use_trend_exit": False, "trend_exit_fee": -1.0, "ts_activation": 1.0, "ts_callback": 0.50, "vol_window": 10, "rsi_threshold": 30, "rsi_max": 50, "use_rsi_drop": True, "vol_factor": 1.0, "use_btc_filter": False},
    "KRW-NEAR": {"interval": "minute60", "ma_short": 3, "ma_long": 15, "use_pullback": False, "use_trend_exit": True,  "trend_exit_fee": -1.5, "ts_activation": 1.5, "ts_callback": 0.25, "vol_window": 10, "rsi_threshold": 30, "rsi_max": 50, "use_rsi_drop": True, "vol_factor": 1.0, "use_btc_filter": False},

    # ⚡ [15분봉(minute15) 스나이퍼 그룹] - 적당한 파동에서 타점을 노리는 코인
    "KRW-BTC":  {"interval": "minute15", "ma_short": 5, "ma_long": 20, "use_pullback": True,  "use_trend_exit": False, "trend_exit_fee": -1.0, "ts_activation": 1.0, "ts_callback": 0.50, "vol_window": 10, "rsi_threshold": 30, "rsi_max": 50, "use_rsi_drop": True, "vol_factor": 1.0, "use_btc_filter": False},
    "KRW-XRP":  {"interval": "minute15", "ma_short": 3, "ma_long": 15, "use_pullback": False, "use_trend_exit": False, "trend_exit_fee": -1.0, "ts_activation": 1.5, "ts_callback": 0.25, "vol_window": 10, "rsi_threshold": 30, "rsi_max": 50, "use_rsi_drop": True, "vol_factor": 1.0, "use_btc_filter": False},
    "KRW-SOL":  {"interval": "minute15", "ma_short": 3, "ma_long": 15, "use_pullback": True,  "use_trend_exit": False, "trend_exit_fee": -1.0, "ts_activation": 1.5, "ts_callback": 0.50, "vol_window": 10, "rsi_threshold": 30, "rsi_max": 50, "use_rsi_drop": True, "vol_factor": 1.0, "use_btc_filter": False},
    
    "KRW-ETH":  {"interval": "minute15", "ma_short": 5, "ma_long": 20, "use_pullback": True,  "use_trend_exit": True,  "trend_exit_fee": -1.5, "ts_activation": 1.5, "ts_callback": 0.25, "vol_window": 10, "rsi_threshold": 30, "rsi_max": 60, "use_rsi_drop": True, "vol_factor": 1.0, "use_btc_filter": True},
    "KRW-LINK": {"interval": "minute15", "ma_short": 3, "ma_long": 15, "use_pullback": False, "use_trend_exit": False, "trend_exit_fee": -1.0, "ts_activation": 1.0, "ts_callback": 0.25, "vol_window": 10, "rsi_threshold": 30, "rsi_max": 50, "use_rsi_drop": True, "vol_factor": 1.0, "use_btc_filter": True}
}
...
# 💡 [수정] RSI 계산 시에도 코인별 독립된 타임프레임(interval) 적용
def get_rsi(ticker, interval="minute15"):
...
print("🚀 UTD_v8_1 멀티 타임프레임(15분/1시간봉) 실전 가동 시작")
send_discord_msg("🤖 **UTD_v8_1** 가동 시작\n- 🕒 멀티 타임프레임 장착 완료 (TAO,NEAR: 1시간 / 나머지: 15분)\n- 불필요한 노이즈 차단 및 최적 손익비 세팅 적용")

while True:
    try:
        now = datetime.now()
        
        # [글로벌 BTC 필터] (현재 V8 세팅에서는 모두 False로 꺼두었으나, 프레임워크는 유지)
        ...
        for ticker in target_tickers:
            ...
            coin_interval = config['interval'] # 💡 각 코인에 맞는 타임프레임 꺼내기
            
            # 💡 5분봉 대신 코인별 맞춤 봉(15분 or 60분) 데이터 호출
            df = pyupbit.get_ohlcv(ticker, interval=coin_interval, count=40)
            ...
            # --- [매도 로직] ---
                ...
                trend_exit_fee = config['trend_exit_fee'] # 💡 코인별 추세 인내심 로드
