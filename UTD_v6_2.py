...
# --- [V6.2 백테스트 기반 최적화 설정 (MA5/20 기반)] ---
# ADA, DOT 제거 및 코인별 최적 낙주/거래량 세팅 반영
strategy_config = {
    # 🥇 효자 종목: 낙주 매수 적극 활용
    "KRW-NEAR": {"rsi_threshold": 30, "rsi_max": 55, "ts_activation": 1.5, "use_rsi_drop": True,  "vol_factor": 1.0},
    "KRW-BTC":  {"rsi_threshold": 30, "rsi_max": 60, "ts_activation": 1.0, "use_rsi_drop": True,  "vol_factor": 0.5},
    
    # 🛡️ 방어형 종목: 낙주 끄고 골든크로스(추세)만 추종
    "KRW-XRP":  {"rsi_threshold": 25, "rsi_max": 55, "ts_activation": 1.5, "use_rsi_drop": False, "vol_factor": 1.0},
    "KRW-AVAX": {"rsi_threshold": 25, "rsi_max": 55, "ts_activation": 1.0, "use_rsi_drop": False, "vol_factor": 1.0},
    "KRW-LINK": {"rsi_threshold": 25, "rsi_max": 55, "ts_activation": 1.0, "use_rsi_drop": False, "vol_factor": 1.0},
    
    # ⚠️ 요주의 종목: 거래량 기준을 살짝 높여서 깐깐하게 진입
    "KRW-ETH":  {"rsi_threshold": 25, "rsi_max": 60, "ts_activation": 1.5, "use_rsi_drop": False, "vol_factor": 0.5},
    "KRW-SOL":  {"rsi_threshold": 25, "rsi_max": 55, "ts_activation": 1.5, "use_rsi_drop": False, "vol_factor": 1.5}
}
...
print("🚀 UTD_v6_2 실전 가동 시작 (MA5/20 & 최적화 타점)")
send_discord_msg("🤖 **UTD_v6_2** 가동 시작\n- MA5/MA20 조합으로 휩쏘 방어 적용\n- 코인별 개별 최적화 파라미터 반영\n- 종목당 25만 원")
    ...
    try:
        ...
        for ticker in target_tickers:
            ...
            # --- [매도 로직] ---
                ...
                # 💡 [핵심] 5선이 20선을 뚫고 내려갈 때만 추세 탈출!
                elif ma5.iloc[-1] < ma20.iloc[-1] and profit_rate < ts_act:
