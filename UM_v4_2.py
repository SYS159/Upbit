...
def get_btc_daily_status():
    """비트코인(BTC) 오늘 09시 시가 대비 양봉/음봉 상태 조회"""
    try:
        url = "https://api.upbit.com/v1/ticker?markets=KRW-BTC"
        res = requests.get(url, timeout=5)
        data = res.json()[0]
        
        # 업비트는 매일 09시 기준으로 opening_price(시가)와 signed_change_rate(부호 있는 변화율)를 제공합니다.
        change_rate = data['signed_change_rate'] * 100 
        current_price = data['trade_price']
        
        if change_rate > 0:
            return f"📈 양봉 (+{change_rate:.2f}%) / `{current_price:,}원`"
        elif change_rate < 0:
            return f"📉 음봉 ({change_rate:.2f}%) / `{current_price:,}원`"
        else:
            return f"➖ 보합 (0.00%) / `{current_price:,}원`"
    except Exception as e:
        print(f"BTC 조회 에러: {e}")
        return "⚠️ 상태 알 수 없음"
    ...
    try:
        ...
        if total_eval is not None:
            ...
            if now.minute in [0] and current_hm != last_sent_hm:
                btc_status = get_btc_daily_status() # 💡 비트코인 상태 가져오기
                
                # 보유 종목이 없어서 details가 비어있을 경우를 위한 깔끔한 처리
                details_text = individual_details if individual_details else "> 텅~ (현재 보유 종목 없음)\n"
